...
static _xvolatile rb_msec_t dfs_current_msec;
...
static fi_cache_mgmt_t *fi_cache_mgmt_new_init(int num);
...
int update_fi_mkdir(fi_inode_t *fin);
...
// init timer
// init g_checkpoint_q
int nn_file_index_worker_init(int max_num)
{
//    conf_server_t *conf = (conf_server_t *)cycle->sconf;

    // 初始化fi_cache_mgmt_t fcm
    g_fcm = fi_cache_mgmt_new_init(max_num);
    if (!g_fcm)
    {
        return DFS_ERROR;
    }

    if (fi_cache_mgmt_timer_new(g_fcm) != DFS_OK)
    {
        return DFS_ERROR;
    }
...
static fi_cache_mgmt_t *fi_cache_mgmt_new_init(int num)
{
    size_t index_num = dfs_math_find_prime(num);
